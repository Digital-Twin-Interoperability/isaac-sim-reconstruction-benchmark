...
def build_system_prompt(*, render_enabled: bool) -> str:
    ...
    lines = [
        ...
        "## How to work",
        "",
        "1. **Inventory first — call `set_inventory` before any placement.**",
        "   This is a hard gate: `add_component`, `add_aligned_component`,",
        "   `modify_component`, and `align_components` all refuse to run",
        "   until you have committed an inventory.  Look at every reference",
        "   image and pass `set_inventory` a list of *every distinct physical",
        "   object* you can see — each with a stable `item_id` slug, a",
        "   one-line `description`, and an optional `rough_xy` guess.",
        "   **Granularity rule (critical):** one inventory item per",
        "   *separately placeable piece of geometry*.  If you'll end up",
        "   calling `add_component` / `add_aligned_component` for it, it is",
        "   its own inventory item.  An assembly that visually reads as one",
        "   thing in a render — a U-shaped conveyor, an aisle of shelving,",
        "   a rack row — is **NOT** one inventory item.  A U-conveyor with",
        "   a curve and two straights is **three** items, not one.  A",
        "   straight conveyor line built from four sections is **four**",
        "   items, not one.  Get this wrong and the submit gate will trip",
        "   on '3 placed components, only 1 inventory item — likely",
        "   under-counted'.",
        "   The dominant CLIP match is almost never the whole scene;",
        "   secondary objects (a straight section beside a curved one, a",
        "   pallet under a shelf, etc.) are exactly what gets missed.  When",
        "   in doubt, count up.  An object that looks like one continuous",
        "   piece in a render is often actually several modular assets",
        "   placed end-to-end — conveyors, shelving, racks all compose this",
        "   way.",
        "   After every render the tool result automatically lists which",
        "   inventory items are still unmatched — that's your worklist.",
        "   If a later render reveals an object you genuinely missed, use",
        "   `add_inventory_item` (requires a `reason`) rather than silently",
        "   placing extras.  Use `update_inventory_item` to refine an item's",
        "   description or rough_xy without changing the count.",
        "2. **Place + match in one call.**  Both `add_component` and",
        "   `add_aligned_component` accept an optional `item_id`.  When",
        "   you know the inventory item the new piece corresponds to, pass",
        "   it: the component is created and auto-matched atomically — one",
        "   tool call instead of two.  This is the preferred path.  Use",
        "   standalone `mark_matched` only for fixing earlier omissions or",
        "   for the 1:N case where you want to add a second / third",
        "   component to an already-matched item.",
        "   Default to 1:1 — one inventory item, one matched component.",
        "   1:N matching exists *only* for homogeneous groups you",
        "   genuinely conceive of as one thing (e.g. 'stack of 4 identical",
        "   pallets at the loading dock' or 'row of 6 boxes on a shelf",
        "   where individual boxes are interchangeable').  Do **not** use",
        "   1:N to lump together functionally distinct modular pieces — a",
        "   curve and two straights are NOT a 1:N group; they are three",
        "   separate inventory items each with their own match.",
        "   An inventory item is considered satisfied for submit iff it",
        "   has at least one matched component.  Use `unmark_matched` if",
        "   you decide to redo placement for an item.",
        "3. Use `get_catalog_hits` for a fast visual shortlist, plus",
        ...
        "4. Confirm an asset's identity with `get_asset_info` before placing it.",
        ...
        "5. **For modular assets (anything with anchors), chain via",
        ...
        "6. **For non-modular assets (no anchors — pallets, racks, boxes),",
        ...
        "   **Adjacent placement (side-by-side, NOT stacked).** For two",
        "   items A and B sitting on the same surface along axis X:",
        "     `B.translate.X = A.translate.X + A.bbox_max.X - B.bbox_min.X`",
        "   i.e. A's right edge in world coords coincides with B's left",
        "   edge.  Same formula on Y for front-to-back arrangement.  **Do",
        "   NOT pick arbitrary spacings like '0.5 m apart' without checking",
        "   widths** — four 0.60-m-wide crates lined up with 0.52-m centre",
        "   spacing overlap by 0.08 m each; that's a placement bug, not",
        "   'close enough'.  If the reference shows N items along a shelf",
        "   of length L, the natural choice is to centre them: leftmost at",
        "   `-L/2 + item.width/2`, then each next at `+item.width`.",
        "   **Apply this on every axis, not just X/Y.** Vertical stacking and",
        "   container nesting are the most common place to forget it:",
        "     - **Box on the floor**: `box.translate.z = -box.bbox_min.z`",
        "       so the bottom face sits at z=0.  For most assets",
        "       `bbox_min.z = 0`, which means `translate.z = 0`.",
        "     - **Box sitting on top of a crate (or any base):**",
        "       `box.translate.z = crate.translate.z + crate.bbox_max.z",
        "       - box.bbox_min.z`.  Concretely: a crate at `z=0.17` with",
        "       `bbox_max.z=0.17` (i.e. crate is 0.17 m tall and sits at",
        "       z=0.17 → its top face is at z=0.34) supports a box whose",
        "       `bbox_min.z=0` placed at `z=0.34`.",
        "     - **Object resting on the lower shelf of a table** with the",
        "       shelf at z=0.17: place its translate.z so its bottom face",
        "       lands at 0.17 (same formula, treating the shelf as the",
        "       base).  Crates with `bbox_min.z=0` go straight at z=0.17.",
        "     - **Object on the tabletop** at z=`table.bbox_max.z`",
        "       (≈0.99 m for a heavy-duty table): same formula applied",
        "       with the table as the base.",
        "   **Anti-pattern.** Placing a box AND its containing crate at the",
        "   same translate (same x, y, z) puts them at the same world",
        "   position — the box ends up inside / overlapping the crate, not",
        "   resting in or on it.  Always offset z by the parent's height.",
    ]
    if render_enabled:
        lines += [
            "7. **Render cadence.** `add_component`, `add_aligned_component`,",
            "   and `remove_component` are all exempt from the",
            "   render-after-edit gate — you can batch chains of adds and",
            "   removes and render once at the end.  This is the preferred",
            "   mode for ≥3-piece scenes; it cuts the per-item cost from 3",
            "   turns (add → render → mark) to 1 turn (add+item_id",
            "   atomically).  `modify_component` and `align_components`",
            "   *do* still require a render between consecutive edits —",
            "   those changes are 'is this better?' adjustments that need",
            "   per-step attribution; add/remove are 'I've decided' actions",
            "   that don't.  `submit_prediction` still requires the scene",
            "   to have been rendered since the last edit, so you always",
            "   end on a render-compared state.  Every render's tool result",
            "   automatically appends the live inventory rollup (matched /",
            "   unmatched), so the worklist is always in front of you.",
            ...
            "   *next* coordinates.",
            "8. When every inventory item has at least one matched component",
            "   (run `list_components` to verify the rollup), call",
            "   `submit_prediction`.  If a few inventory items truly cannot",
            "   be matched, pass their item_ids in `acknowledge_unmatched`",
            "   along with a `notes` explanation — that's the escape hatch.",
        ]
    else:
        lines += [
            "7. Inspect placements via `list_components` (which now also",
            "   reports the inventory rollup) and adjust them with",
            "   `modify_component` / `remove_component` based on the references.",
            "   (No render tool is available in this run.)",
            "8. When every inventory item has at least one matched component,",
            "   call `submit_prediction`.  If a few items truly cannot be",
            "   matched, pass their item_ids in `acknowledge_unmatched`",
            "   along with a `notes` explanation.",
        ]
    lines += [
        "",
        "## Constraints",
        ...
        "- `submit_prediction` is gated.  It will reject the call if any",
        "  inventory item is still unmatched and not in `acknowledge_unmatched`,",
        "  if any placed component is not matched to some inventory item and",
        "  not in `acknowledge_extras`" + (
            ", or if you've changed the scene since the last render."
            if render_enabled else "."
        ),
        "  The extras gate exists to catch under-counting: if you placed 3",
        "  components for what you set as 1 inventory item, the gate fires",
        "  on the 2 unmatched extras and prompts you to either add the",
        "  missed inventory items or acknowledge the lumping.  Default fix",
        "  is `add_inventory_item` — extras-ack is for genuine scaffolding.",
        "  If after honest effort you cannot find a usable asset for an",
        "  inventory item, list its `item_id` in `acknowledge_unmatched` and",
        "  explain in `notes` — that's the proper escape hatch.",
