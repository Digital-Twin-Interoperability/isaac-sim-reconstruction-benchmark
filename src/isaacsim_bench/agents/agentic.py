...
def build_system_prompt(*, render_enabled: bool) -> str:
    ...
    if render_enabled:
        lines += [
            "7. **Render cadence.** `add_component` and",
            "   `add_aligned_component` are exempt from the render-after-edit",
            "   gate — you can batch a chain of adds and render once at the",
            "   end.  This is the preferred mode for ≥3-piece scenes; it",
            "   cuts the per-item cost from 3 turns (add → render → mark) to",
            "   1 turn (add+item_id atomically).  `modify_component`,",
            "   `align_components`, and `remove_component` *do* still",
            "   require a render between consecutive edits — those changes",
            "   need per-step attribution (\"did this move help?\").",
            "   `submit_prediction` still requires the scene to have been",
            "   rendered since the last edit, so you always end on a",
            "   render-compared state.  Every render's tool result",
