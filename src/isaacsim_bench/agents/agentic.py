...
def build_system_prompt(*, render_enabled: bool) -> str:
    ...
    lines = [
        ...
        "2. **After placing each component, call `mark_matched`** to tie it",
        "   to its inventory item.  Default to 1:1 — one inventory item,",
        "   one matched component.  1:N matching exists *only* for",
        "   homogeneous groups you genuinely conceive of as one thing",
        "   (e.g. 'stack of 4 identical pallets at the loading dock' or",
        "   'row of 6 boxes on a shelf where individual boxes are",
        "   interchangeable').  Do **not** use 1:N to lump together",
        "   functionally distinct modular pieces — a curve and two",
        "   straights are NOT a 1:N group; they are three separate",
        "   inventory items each with their own match.",
    ...
    if render_enabled:
        lines += [
            "7. **Edit one thing, then render.** Each scene edit —",
            ...
            "   or worse.  Every render's tool result automatically appends",
            "   the live inventory rollup (matched / unmatched), so the",
            "   worklist is always in front of you — no need to re-derive it.",
